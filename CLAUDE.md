...
## YouTube Thumbnail (썸네일)

각 에피소드 루트에 `thumbnail.png` (1280×720) 생성. **좌우 분할 레이아웃**이 표준.

### 레이아웃 규칙
- **왼쪽 크라프트 패널** (~55%, x=0~700): 단어 + 핀인 + 영어 의미. 단색 배경이라 글자 대비 최대.
  - 단어: `fnt_cn(230, bold)`, ACCENT 색, 패널 중앙 정렬
  - 핀인: `fnt_cn(80, bold)`, MID 색 / 액센트 가로줄 / 영어: `fnt_cn(46, bold)`, MID
- **오른쪽 사진** (~45%, x=700~): cover-crop으로 꽉 채움. **콜라주(00b)에 쓴 사진과 동일한 ref 이미지 사용** (영상 톤 통일).
  - 사진 2장이면 위아래 분할(가운데 6px 갭), 1장이면 전체.
- **이음새**: 사진 왼쪽에 그라데이션 그림자(폭 60px, 알파 90→0) + 5px ACCENT 세로 라인.

### 핵심 원칙
- **글자를 사진 위에 직접 올리지 않는다** — 글자는 크라프트 패널, 사진은 분리 → 대비·가독성·사진 노출 모두 확보.
- 카드 생성기 폰트·색(`card_generator` ACCENT/MID, `fnt_cn`) 재사용 → 타이틀 카드와 브랜드 일관성.
- 모바일 작은 화면 기준으로 단어가 충분히 크게.
